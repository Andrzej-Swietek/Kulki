 ...
 * @param values
 */
export function create(strings, classList: string | Array<string>, ...values) : HTMLElement {
    const element: HTMLElement = document.createElement(strings[0]);
    if (classList instanceof Array)
       classList.forEach( (className: string) => element.classList.add(className) )
    else
